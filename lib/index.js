          ...
          if(fileFormat === 'image') {
            ...
            if (fileType !== 'thumbnail') {
              for (let size of imageSizes) {
                ...
                console.log(`🔄 Genereted ${fileFormat}s/${size.name}/${file.name}${file.ext}`);
                ...
                if (size.isGenerateWebp && file.ext !== '.webp') {
                  buffer = await Sharp(file.buffer)
                    ...
                    .resize(size.resizeOptions || {})
                    .toBuffer();
                    console.log(`🔄 Genereted ${fileFormat}s/webp/${size.name}/${file.name}.webp`);

                  buffers.push({
                    buffer,
                    path: `webp/${size.name}/${file.hash}.webp`,
                    mime: 'image/webp',
                  });
                }
              }
            ...
            for (let item of buffers) {
              await S3.upload(
                ...
                (err, data) => {
                  ...
                  console.log(`✅ Uploaded ${fileFormat}s/${item.path}`);
                }
              );
            }
            ...
            await S3.upload(
              ...
              (err, data) => {
                if (err) return reject(err);
                console.log(`✅ Uploaded ${fileFormat}s/${file.hash}${file.ext}`);
              }
            );
          ...
          if(fileFormat === 'image') {
            await S3.deleteObject(
              ...
              (err, data) => {
                ...
                console.log(`❌ Deleted ${fileFormat}s/${fileType}/${file.hash}${file.ext}`);
              }
            )
            if (file.hash.split('_')[0] !== 'thumbnail') {
              for (let size of imageSizes) {
                if (file.ext !== '.webp') {
                  await S3.deleteObject(
                    ...
                    (err, data) => {
                      if (err) { console.error(err); return; }
                      console.log(`❌ Deleted ${fileFormat}s/${size.name}/${file.hash}${file.ext}`);
                    }
                  )
                }
                if (size.isGenerateWebp) {
                  await S3.deleteObject(
                    { Key: `images/webp/${size.name}/${file.hash}.webp`, ...customParams, },
                    (err, data) => {
                      if (err) { console.error(err); return; }
                      console.log(`❌ Deleted ${fileFormat}s/webp/${size.name}/${file.hash}.webp`);
                    }
                  )
                }
              }
            ...
            await S3.deleteObject(
              ...
              (err, data) => {
                if (err) { console.error(err); return; }
                console.log(`❌ Deleted ${fileFormat}s/${file.hash}${file.ext}`);
              }
            )
          }
